...
$KEY_NAME = Read-Host "Please enter a name for your VPN key file (without extension):"

# Step 2: Function to generate SSH keys
function Generate-SSHKeys {
    param (
        [string]$KeyName  # Accept the key name as a parameter
    )
    
    Write-Host "Generating SSH keys..." -ForegroundColor Yellow
    ssh-keygen -t rsa -f "$HOME\.ssh\$KEY_NAME" "" *>&1 | Out-Null  # Suppress output
    Write-Host "SSH Keys generated!" -ForegroundColor Yellow
}
...
function Transfer-Files {
    ...
    scp "$HOME\.ssh\$KEY_NAME.pub" root@{$SERVER_PUBLIC_IP}:~/.ssh/authorized_keys
...
Generate-SSHKeys -KeyName $KEY_NAME
Transfer-Files
...
Write-Host "ssh -i $HOME\.ssh\$KEY_NAME root@$SERVER_PUBLIC_IP" -ForegroundColor Red
